...
#plt.plot(x,th)
  plt.plot(x/L,th2/th0,linewidth=3.,label=r'$\beta=$ '+'%.0f' %(beta*L))
plt.xlabel('$x /L$')
...
plt.figure()
x=np.linspace(0.0001,L,100)
xi=beta*x

plt.plot(x/L,np.tanh(xi)/xi,linewidth=3.,label='$\chi=$ '+'%.0f' %(beta*L))
plt.xlabel(r'$\chi= \beta L$')
plt.ylabel(r'$\eta$ ')
plt.grid()
plt.show()
